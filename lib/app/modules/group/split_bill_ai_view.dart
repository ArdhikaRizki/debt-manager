...
class _SplitBillAiViewState extends State<SplitBillAiView> {
  ...
  Future<void> _submitTransactions() async {
    ...
    for (var item in _items) {
      ...
      if (assigned.isEmpty)
        continue; // Skip kalau item tidak ada yang menanggung

      final price = item['price'] as double;
      final splitPrice =
          price / assigned.length; // Harga dibagi rata jumlah orang yang assign
    ...
    // Kembali langsung ke halaman transaksi grup
    Get.until((route) {
      final name = route.settings.name;
      return name == AppRoutes.groupTransaction || name == AppRoutes.group;
    });
    ...
    Get.snackbar(
      'Berhasil!',
      '$successCount Transaksi Split Bill otomatis tercatat.',
  ...
  Widget build(BuildContext context) {
    ...
    final members =
        widget.txController.group.value?.members
    ...
    return Scaffold(
      ...
      bottomNavigationBar: _isLoading || _errorMsg.isNotEmpty
          ? null
          : _buildBottomBar(),
    );
  }
          ...
          children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: (grandTotal > 0 && !_isSubmitting)
                    ? _submitTransactions
                    : null,
                style: ElevatedButton.styleFrom(
                  backgroundColor: AppColors.primaryTeal,
